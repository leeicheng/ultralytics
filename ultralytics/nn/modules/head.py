...
from ultralytics.utils.tal import TORCH_1_10, dist2bbox, dist2rbox, make_anchors, dist2point
...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment" ,"PointDetect"
...
class PointDetect(Detect):
    """YOLO PointDetect head for keypoint detection models."""
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.nl = len(ch)
        self.reg_max = 16
        self.no = nc + self.reg_max * 2
        self.stride = torch.zeros(self.nl)
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 2)), max(ch[0], min(self.nc, 100))
        self.cv2 = nn.ModuleList(
    ...
    def forward(self, x):
        ...
        Returns raw predictions for training, and calls _inference for evaluation.
        """
        for i in range(self.nl):
            x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
    ...
    @staticmethod
    def postprocess(preds: torch.Tensor, max_det: int, nc: int = 80):
        """Post-processes YOLO model predictions for KEYPOINTS."""
        batch_size, anchors, _ = preds.shape
        points, scores = preds.split([2, nc], dim=-1)
        index = scores.amax(dim=-1).topk(min(max_det, anchors))[1].unsqueeze(-1)
        points = points.gather(dim=1, index=index.repeat(1, 1, 2))
        scores = scores.gather(dim=1, index=index.repeat(1, 1, nc))
        scores, index = scores.flatten(1).topk(min(max_det, anchors))
        i = torch.arange(batch_size, device=preds.device)[..., None]
        return torch.cat([points[i, index // nc], scores[..., None], (index % nc)[..., None].float()], dim=-1)
