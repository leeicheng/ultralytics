...
class PointDetectionModel(BaseModel):
    """YOLO detection model."""
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, PointDetect}
        ):
...
def guess_model_task(model):
    ...
    Returns:
        (str): Task of the model ('detect', 'segment', 'classify', 'pose', 'obb', 'point').
    """
